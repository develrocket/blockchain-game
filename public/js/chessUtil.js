/** This is a set of utilities for chess. The functions here do not
 *  have a persistent game state--it is passed as a first argument to
 *  any function that needs it--so they cannot account for certain
 *  state-based rules like not castling if the king or rook in
 *  question have moved. The game state is always assumed to be an
 *  object with a property for every square, so that gameState[square]
 *  tells us what sort of piece is at that position. The peice is
 *  represented by a simple two character string with the first
 *  character being the color ("b" or "w") and the second being the
 *  type ("p", "R", "N", "B", "Q" or "K"). The ChessUtil object is
 *  really just a namespace for the whole set of functions.
 */
var ChessUtil = {
  ...
  /** Returns the specified move in algebraic notation. If the
   *  specified move is not valid, null will be returned. The
   *  gameState provided has to be the state BEFORE the move and is
   *  used to account for possible ambiguity.
   */
  ...
  /** Returns the next square from the given square in the current
   *  direction or, if a number is supplied, returns the square that
   *  many away from the current square in the specified direction.
   *  If the square is not valid, null is returned. The direction has
   *  to be "n", "s", "w", "e", "nw", "ne", "sw", "se". If it is not,
   *  null is returned.
   */
  ...
  /** Checks whether the king of the specified color is in check in
  ...
  /** Returns whether the specified move in the specified game state
   *  results in check on the specified color.
   */
  ...
  /** Returns an array of booleans corresponding to whether each move
   *  in the moves array will lead to that color's king being in
   *  check. True means that that move will lead to check; false that
   *  it will not.
   */
  ...
  /** Performs the move on the specified game state, returning the
   *  resulting state. The move should be in algebraic notation, with
   *  an optional color argument that is not optional if the move is
   *  something ambiguous, like castling. If the move, for some
   *  reason, is invalid, null is returned.
   */
  ...
  /** Returns whether the specified square in the given game state is
   *  attack by the color in question.
   */
  ...
  /** Returns a piece object based on the piece at the given position
   *  in the given game state. This will return null if there is no
   *  piece at the specified position.
   */
  ...
  /** Gets the possible moves for the specified piece. By default,
   *  these are returned as an array. The function takes two
   *  arguments--a game state in the form of a hash as per
   *  Game#getGameState() and a position which has to be valid. If the
   *  position is not valid, null will be returned. If the game state
   *  is not valid, it'll probably throw an error. The two mandatory
   *  arguments can be followed by a list of options in the for of an
   *  object literal like "{checkForCheck : true}" or, of course, just
   *  an object with those properties defined as necessary. This list
   *  of options can change what the function does at a very
   *  fundamental level: they can change the returned type (only to
   *  boolean, currently) and can also change its time and memory
   *  requirements extensively.
   *   
   *  Only the following options are currently recognized by this
   *  function: checkForCheck : boolean -- if set to true, this will
   *  return whether the specified piece puts the enemy king in check.
   *  checkSquare : position -- checks whether the specified square is
   *  attacked by the piece. If checkForCheck is set to true, this is
   *  ignored.  hasMoves : boolean -- sets whether to just check *if*
   *  there are any moves, without actually getting all of them. This
   *  returns a boolean and should be faster than getting the entire
   *  list of moves then checking the length. This one supercedes both
   *  checkForCheck and checkSquare.
   *  
   *  All the other ChessUtil move-finding functions accept these
   *  options, although some may accept more options.
   */
  ...
  /** Returns an array of moves with all the moves that would result
   *  in check (and are therefore illegal removed.
   */
  ...
  /** Returns all the possible moves for a pawn, given the pawn and a
   *  game state. If the piece is not in the game state, null is
   *  returned. Otherwise, an array of possible moves, in the form of
  ...
  /** Returns all the possible moves for the specified
   *  knight. Otherwise is the same as ChessUtil.pawnMoves().
   */
  knightMoves : function (gameState, position, options) {
    ...
    /** Returns either null (do nothing), a move (push it to the
     *  array) or a boolean (return it).
     */
    function check() {
  ...
  /** Gets the moves for a piece that moves in a line, like a rook,
   *  bishop or queen. This function has the same arguments as any
   *  other, but accepts more options than the others. These new
   *  options are directions, an array of directions that must all be
   *  valid, and limit, the maximum distance the piece can go in a
   *  straight line. For most pieces a limit of 8 is fine; the king,
   *  obviously, has a limit of 1. Limit is 8 by default, but
   *  directions _must_ be specified.
   */
  ...
  /** Gets the moves for the specified bishop; works just like any of
  ...
  /** Gets the moves for the specified rook; works just like any of
   *  the other, similar functions.
   */
  ...
  /** Gets the moves for the specified queen; works just like any of
   *  the other, similar functions.
   */
  ...
  /** Gets the moves for the specified king; works just like any of
   *  the other, similar functions, but also accounts for the
   *  possibility of castling, albiet non-deterministically--due to
   *  its not having any previous state information, it cannot check
   *  whether the king or rook in question have moved before; as such,
   *  it is up to the game itself to ultimately decide if castling is
   *  legal.
   */
  kingMoves : function (gameState, position, options) {
    ...
    /** Returns whether the specified file's home-row square is
     *  attacked.
     */
