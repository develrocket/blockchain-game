...
var Game = Class.create({
  ...
  initialize : function (holder) {
    ...
    /** Adds the given listener to the list, to be notified if there
     *  is a change in game state. This does not do any checks of the
     *  listener's validity.
     */
    ...
    /** Removes the specified listener from the list of listeners if
     *  it is there, doing nothing otherwise.
     */
    ...
    /** Fires the specified event, notifying all of the listeners
     *  about it. The argument tells the event whether it was caused
     *  by a game reset (true) or a normal move
    ...
    /** Returns whether the specified side is on. If a side is on, it
     *  is controlled by a player locally.  otherwise, the side is
     *  controlled by something else, hopefully, or nothing at all:
     *  the player, either way, cannot control it. If the given side
     *  is not valid, undefined will be returned.
     */
    this.isSideOn = function (side) {
      return sidesOn[side];
    };

    /** Toggles whether a particular side is on. The side supplied has
     *  to be either "black" or "white"; otherwise, nothing
     *  happens.
     */
    ...
    /** Sets the given side on or off. If the side is already in the
     *  specified state, nothing happens.  If any of the given
     *  arguments are not valid, nothing happens. The state must be a
     *  boolean--true means that the side should be turned on;
     *  false--off. The side has to be represented by a string that
     *  begins with either "w" or "b", corresponding to white and
     *  black, respectively. Case does not matter.
     */
  ...
  /** Creates the board, which is a bunch of Squares of alternating
   *  colors all properly arranged, as they would be on an actual
   *  chess board.  Also creates the axis labels that mark the files
   *  and ranks by name.
   */
  setUpBoard : function () {
    ...
    /** Returns the square at the specified position or null if there
     *  is none.
     */
  ...
  setUpJail : function () {
    ...
    /** Removes all the pieces in the jail from the DOM. Basically,
     *  they die. Forever. Mourn them!
     */
    jail.clear = function () {
  ...
  setUpHistory : function () {
    ...
    upButton.insert(new Element("img", {"src" : "images/upTriangle.png", "alt" : "Up"}));

    var downButton = new Element("div", {"class" : "disabled average button"});//The button to scroll down.
    downButton.insert(new Element("img", {"src" : "images/downTriangle.png", "alt" : "Down"}));
    ...
    /** Checks which of the scroll buttons should be enabled and
     *  enables them as necessary. "Necessary" is a word that always
     *  annoys me.*/
  ...
  /** Sets up the little menu on the bottom-right with the new game
   *  buttons.
   */
  setUpMenu : function() {
	...
	this.removeGameStateListener(moveMade);
      ...
      /** This function responds to moves made, giving them to the ai
  ...
  /** Returns the amount of points the specified color has on the
   *  board currently.
   */
  ...
  /** Promotes the specified piece by showing the player a menu where
   *  they can choose what piece the pawn will now become. If the
   *  specified piece is, for some reason, not valid, nothing will
   *  happen.
   */
  ...
  /** Changes the turn from white to black and vice versa. This both
   *  changes the turn from the game's perspective--the opposite side
   *  gets to move its pieces--and does some things with the actual
   *  interface, namely changing the background, getting rid of any
   *  selected squares and checking to see if the game is over due to
   *  a checkmate or a stalemate. If the argument is not falsey, then
   *  the event after the change of background is fired as a "new
   *  game" type of event.
   */
  ...
  /** Sets the turn to white, calling changeTurn() if it is currently
   *  black's turn.
   */
  resetTurn : function () {
    if (this.turn == "black") {
  ...
  /** Sets the selected piece, making sure only one is selected at a
   *  time and taking care of the visual selecting the pieces
   *  use. Once the piece is selected, all its possible moves are
   *  found and shown on the board; when it is deselected, any shown
   *  moves are hidden.
   */
  ...
  /** Returns whether the king whose turn it is can currently castle
   *  the specified way ("0-0-0" or "0-0").
   */
  ...
  /** Makes the specified move. This method will do nothing if the
   *  piece's color does not match the turn.  or if the given move is
   *  not valid. Leading and trailing whitespace is ignored and the
   *  delimitter between the initial square and the final square can
   *  be any one character that is not used otherwise.
   */
  ...
  /** Moves the specified piece to the new position. If either of the
   *  arguments is invalid, does nothing.
   */
  ...
  /** Removes the specified piece from the board; if the piece is not
  ...
  /** Returns the current state of the game as a hash with string
   *  representing each piece as values and all the possible square
   *  positions as keys. Any key without a piece will contain null.
   */
  ...
  /** Returns the old game state which is the game state before the
   * last move was carried out.
   */
...
/** This class encapsulates information about a change of game
 *  state. It contains the new game state as well as whether it is a
 *  new game (whether the change in state came about because of
 *  game.reset()).
 */
var GameStateChangedEvent = Class.create({
  initialize: function(oldState, state, move, turn, newGame) {
    ...
    /** Returns a boolean specifying whether the change in state is
     *  due to the game's being reset.
     */
...
var Sidebar = Class.create({
  initialize : function (options) {
    ...
    if (options.direction != "right") {
      bar = new Element("img", {"src" : "images/rightBar.png", "class" : "bar"});
      bar.setStyle("float : right");
    } else {
      bar = new Element("img", {"src" : "images/leftBar.png", "class" : "bar"});
    }
    ...
    var highlight = function () {
      ...
      curr = curr.replace("images/", "images/highlighted");
      console.log(curr);
      bar.setAttribute("src", curr);
    };
    var dehighlight = function () {
      var curr = bar.getAttribute("src");
      curr = curr.replace("images/highlighted", "images/");
      console.log(curr);
      bar.setAttribute("src", curr);
    }
...
/** This class represents a piece. It can be black or white and can be
 *  in a square. The piece's element is the div that contains it along
 *  with the appropriate image.
 */
var Piece = Class.create({

  /** Creates and places the piece to the specified position, which is
   *  also its id.
   */
  initialize: function(type, color, position, parent) {
	...
	return;
      ...
      type = newType;
      img.writeAttribute("src", "images/" + color.substring(0, 1) + type + ".png");
    };
    ...
    this.setSelected = function (isSelected) {
      ...
      if (selected) {
	this.element.setStyle({"background": "url(images/sl.png)"});
      } else {
    ...
    picture = "images/" + color.substring(0,1) + type + ".png";//The picture's URL.
  ...
  /** Returns a string representation of the object with the first
   *  letter of the color followed by the type.  For a black knight,
   *  it would return "bN".
   */
  toString: function() {
  ...
  /** Highlights the pieces, as when somebody hovers their mouse over
   *  it. Does nothing if the piece is selected.
   */
  highlight: function() {
    //This will do nothing if the piece is selected or not on the board.
    if (!this.isSelected() && this.getPosition()) {
      this.element.setStyle({"background": "url(images/hl.png)"});
    }
  },

  /** Dehighlights the piece, as when the mouse leaves it. Does
   *  nothing if the piece is selected.
   */
...
var Square = Class.create({
  ...
  /** Creates the square with the specified color and position as part
   *  of the specified game (parent).
   */
  ...
  /** This function sets this square's piece. If the square is
   *  empty--it contains no piece--then this method will set the
   *  piece, actually putting the piece into this square's div as
   *  well. If there is a piece already, this method (currently) does
   *  nothing.
   */
  ...
  //------------------------------Highlighting, selecting and stuff:------------------------------//
  /** Highlights the square, chaning it's background image
   *  appropriately. This also start to watch the square for when it
   *  is hovered over, in order to actively select it in such a
   *  case.
   */
  highlight: function() {
    this.highlighted = true;

    this.element.setStyle({backgroundImage: "url(images/hls.png)", cursor: "pointer"});
    this.element.observe("mouseover", this.highlightObserver);
  },

  /** Makes the square "actively" highlighted, as when it is
   *  mouseovered while highlighted.
   */
  activeHighlight: function() {
    this.activeHighlighted = true;

    this.element.setStyle({backgroundImage: "url(images/hl.png)", cursor: "pointer"});
    this.element.observe("mouseout",  this.dehighlightObserver);
    this.element.observe("click", this.moveObserver);
  },

  /** Insures that the square is not highlighted. This also deals with
   *  all the variables and observers.
   */
...
/** A chess clock that lets people play timed games locally or against
 *  the computer. Each time the turn starts, that side's time starts
 *  ticking away; the countdown is paused as soon as it is the other
 *  side's turn, and then that side starts losing time. This is just
 *  the model for a clock; it only keeps the time and sends out events
 *  when appropriate--it does not actually display the time
 *  anywhere.
 */
var Clock = Class.create({
  ...
  initialize : function (game) {
    ...
    /** Fires an event notifying all of the listeners that the time
     *  has changed.
     */
